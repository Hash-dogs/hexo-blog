...
**自检**：写完跑一次统计，`「` 和 `——` 应为 0，全角括号 `（` 除代码注释外应为 0：

```bash
python -c "s=open('source/_posts/<slug>.md',encoding='utf-8').read().split('---',2)[2]; print({k:s.count(k) for k in ['「','」','——','（','）']})"
```
...
2. **先跑一遍《标题与措辞规范》的自检**，确认标题、小标题、正文都已去 AI 味（`「」` 和 `——` 为 0，无评价性修饰词），自检结果随博文一起展示
